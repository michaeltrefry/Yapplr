...
# Check production deployment status and what would be rebuilt
...
echo -e "${GREEN}📊 Production Deployment Status Check${NC}"
echo -e "${BLUE}=====================================${NC}"
...
if [ ! -f "$HASH_FILE" ]; then
    echo -e "${YELLOW}⚠️ No production deployment hash file found - all services will be rebuilt${NC}"
    echo ""
fi
...
if [ $needs_rebuild -eq 0 ]; then
    ...
    echo -e "  ${GREEN}⏱️ Estimated time: ~3-4 minutes${NC}"
else
    echo -e "  ${YELLOW}🔨 Next deployment will rebuild $needs_rebuild service(s)${NC}"
    echo -e "  ${YELLOW}⏱️ Estimated time: ~$((3 + needs_rebuild * 4)) minutes${NC}"
fi

echo ""
echo -e "${BLUE}💡 Tips:${NC}"
echo -e "  - Run ${GREEN}./deploy-prod-optimized.sh${NC} to deploy with optimizations"
echo -e "  - Run ${GREEN}./force-rebuild-prod.sh [service]${NC} to force rebuild specific services"
echo -e "  - Run ${GREEN}./force-rebuild-prod.sh all${NC} to force rebuild everything"
echo -e "  - ${GREEN}🛡️ ALL PRODUCTION DATA IS ALWAYS PRESERVED${NC}"

# Check running containers
echo ""
echo -e "${BLUE}🐳 Current Running Production Containers:${NC}"
...
echo ""
echo -e "${BLUE}💾 Production Storage Status:${NC}"
if [ -d "/mnt/yapplr-prod-storage" ]; then
    echo -e "  ${GREEN}✅ Production storage directory exists${NC}"
    storage_size=$(du -sh /mnt/yapplr-prod-storage 2>/dev/null | cut -f1 || echo "unknown")
    echo -e "  ${BLUE}📊 Total storage used: $storage_size${NC}"
else
    echo -e "  ${YELLOW}⚠️ Production storage directory not found${NC}"
fi
