...
public class NotificationService : INotificationService
{
    ...
    private Notifications.QueuedNotification CreateQueuedNotification(NotificationRequest request, int notificationId)
    {
        var queuedNotification = new Notifications.QueuedNotification
        {
            UserId = request.UserId,
            NotificationType = request.NotificationType,
            Title = request.Title,
    ...
    public async Task CreateFollowNotificationAsync(int followedUserId, int followingUserId)
    {
        ...
        // Send real-time notification (without creating another database notification)
        var request = new NotificationRequest
        {
            UserId = followedUserId,
            NotificationType = "follow",
            Title = "New Follower",
            Body = $"@{followingUser.Username} started following you",
            Data = new Dictionary<string, string>
            {
                ["type"] = "follow",
                ["followerUsername"] = followingUser.Username
            },
            SuppressDatabaseNotification = true // Don't create database notification since we already created one above
        };
    ...
    public async Task CreateCommentNotificationAsync(int postOwnerId, int commentingUserId, int postId, int commentId, string commentContent)
    {
        ...
        // Check if the post owner is mentioned in the comment content
        // If they are, skip the comment notification since they'll get a mention notification
        ...
        if (mentionedUsernames.Any())
        {
            ...
            if (postOwner != null && mentionedUsernames.Contains(postOwner.Username.ToLower()))
            {
                // Post owner is mentioned, they'll get a mention notification instead
                return;
            }
        }
        ...
        var request = new NotificationRequest
        {
            ...
            Title = "New Comment",
            Body = $"@{commentingUser.Username} commented on your {mediaTypeText}",
            Data = new Dictionary<string, string>
            {
                ["type"] = "comment",
    ...
    /// </summary>
    public async Task CreateMentionNotificationsAsync(string? content, int mentioningUserId, int postId)
    {
        ...
        // Get the post/comment where the mention occurred to determine type
        var post = await _context.Posts.FindAsync(postId);
        if (post == null)
            return;

        var isCommentMention = post.PostType == PostType.Comment;
        var parentPostId = isCommentMention ? post.ParentId : postId;
        ...
        foreach (var mentionedUser in mentionedUsers)
        {
            ...
            var notification = new Notification
            {
                ...
                PostId = parentPostId, // For comments, this is the parent post; for posts, this is the post itself
                CommentId = isCommentMention ? postId : null, // For comments, this is the comment ID; for posts, this is null
                CreatedAt = DateTime.UtcNow
            };
            ...
            {
                Username = mentionedUser.Username, // Set the required username field
                MentioningUserId = mentioningUserId,
                MentionedUserId = mentionedUser.Id,
                PostId = postId, // This is always the post/comment where the mention occurred
                NotificationId = notification.Id, // Link to the notification we just created
                CreatedAt = DateTime.UtcNow
            };
            ...
            var request = new NotificationRequest
            {
                ...
                Body = $"@{mentioningUser.Username} mentioned you in a {(isCommentMention ? "comment" : "post")}",
                Data = new Dictionary<string, string>
                {
                    ["type"] = "mention",
                    ["postId"] = parentPostId.ToString(),
                    ["mentionerUsername"] = mentioningUser.Username
                },
                SuppressDatabaseNotification = true // Don't create database notification since we already created one above
            };
            ...
            if (isCommentMention)
            {
                request.Data["commentId"] = postId.ToString(); // The comment's own ID
            }
    ...
    public async Task CreateSystemMessageNotificationAsync(int userId, string message)
    {
        ...
        var request = new NotificationRequest
        {
            ...
            Body = message,
            Data = new Dictionary<string, string> { ["type"] = "system_message" },
    ...
    public async Task SendReactionNotificationAsync(int userId, string reactorUsername, int postId, ReactionType reactionType)
    {
        ...
        {
            UserId = userId,
            NotificationType = "like", // Use "like" to match frontend expectations and avoid duplicate notifications
            Title = "Post Reaction",
            Body = $"@{reactorUsername} reacted {reactionEmoji} to your {mediaTypeText}",
            Data = new Dictionary<string, string>
            {
                ["type"] = "like", // Use "like" to match frontend expectations
                ["postId"] = postId.ToString(),
                ["reactorUsername"] = reactorUsername,
                ["reactionType"] = ((int)reactionType).ToString(),
                ["reactionEmoji"] = reactionEmoji
            },
            SuppressDatabaseNotification = true // Suppress database notification since CreateReactionNotificationAsync already created one
        };

        await SendNotificationAsync(request);
    }

    public async Task SendCommentReactionNotificationAsync(int userId, string reactorUsername, int postId, int commentId, ReactionType reactionType)
    {
        var reactionEmoji = reactionType.GetEmoji();
        var request = new NotificationRequest
        {
            ...
            Body = $"@{reactorUsername} reacted {reactionEmoji} to your comment",
            Data = new Dictionary<string, string>
            {
                ["type"] = "like", // Use "like" to match frontend expectations
                ["postId"] = postId.ToString(),
            ...
            SuppressDatabaseNotification = true // Suppress database notification since CreateCommentReactionNotificationAsync already created one
        };
    ...
    public async Task CreateCommentReactionNotificationAsync(int commentOwnerId, int reactingUserId, int postId, int commentId, ReactionType reactionType)
    {
        ...
        // Send real-time notification
        await SendCommentReactionNotificationAsync(commentOwnerId, reactingUser.Username, postId, commentId, reactionType);
    }
    ...
    public async Task<NotificationListDto> GetUserNotificationsAsync(int userId, int page = 1, int pageSize = 25)
    {
        var query = _context.Notifications
            ...
            .Include(n => n.Mention) // Include mention data for mention notifications
            .OrderByDescending(n => n.CreatedAt);
