...
public class FirebaseNotificationProvider : IFirebaseNotificationProvider
{
    ...
    public FirebaseNotificationProvider(YapplrDbContext context, ILogger<FirebaseNotificationProvider> logger, INotificationContentBuilder contentBuilder, IConfiguration configuration, IOptions<NotificationProvidersConfiguration> notificationOptions)
    {
        _context = context;
        _logger = logger;
    ...
    public async Task<bool> SendMessageNotificationAsync(int userId, string senderUsername, string messageContent, int conversationId)
    {
        ...
        return await SendNotificationAsync(user.FcmToken, content.Title, content.Body, content.Data);
    }
    ...
    public async Task<bool> SendFollowNotificationAsync(int userId, string followerUsername)
    {
        ...
        // Add Firebase-specific userId data
        content.WithData("userId", userId.ToString());
        return await SendNotificationAsync(user.FcmToken, content.Title, content.Body, content.Data);
    }
